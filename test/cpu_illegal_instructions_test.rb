...
class Mos6502::CpuIllegalInstructionsTest < Minitest::Test
  ...
  def test_0x83
    ...
    @cpu.step
    @cpu.step
    assert_equal([0x00, 0x40, 0x00], @cpu.dump_memory(0x404, 3))
  end

  # SAX (zero page)
  def test_0x87
    @cpu.load!([0xa9, 0x40, 0xa2, 0xf0, 0x87, 0x20, 0xa9, 0x00, 0x87, 0x20])
    @cpu.step
    @cpu.step
    @cpu.step
    assert_equal([0x00, 0x40, 0x00], @cpu.dump_memory(0x1f, 3))
    @cpu.step
    @cpu.step
  ...
  # SAX (absolute)
  def test_0x8f
    @cpu.load!([0xa9, 0x21, 0xa2, 0xf0, 0x8f, 0x01, 0x20])
    @cpu.step
    @cpu.step
    @cpu.step
    assert_equal([0x00, 0x00, 0x00], @cpu.dump_memory(0x1f, 3))
  ...
  # SAX (zero page, Y)
  def test_0x97
    @cpu.load!([0xa9, 0x00, 0xa2, 0x40, 0xa0, 0xf0, 0x97, 0x20])
    @cpu.step
    @cpu.step
    @cpu.step
    @cpu.step
    assert_equal([0x00, 0x00, 0x00], @cpu.dump_memory(0x0f, 3))
  end
